...
//			$elem[0] = date('Y-m-d H:i', $elem[0]/1000);
//			return $elem;
//		}, $data['Bookings']));
		...
		<script type="text/javascript">
			jQuery( document ).ready( function($) {
				...
				function _flot_tooltip(x, y, contents) {
					$('<div id="sln-flot-tooltip">' + contents + '</div>').css( {
						position: 'absolute',
						display: 'none',
						top: y + 5,
						left: x + 5,
				...
				$("#sln-graph-<?php echo $this->id; ?>").bind("plothover", function (event, pos, item) {
					$("#x").text(pos.x.toFixed(2));
					$("#y").text(pos.y.toFixed(2));
					if (item) {
						if (previousPoint != item.dataIndex) {
							previousPoint = item.dataIndex;
							$("#sln-flot-tooltip").remove();
							var x = item.datapoint[0].toFixed(2),
								y = item.datapoint[1].toFixed(2);
							if( item.series.id == 'earnings' ) {
								if( '<?php echo $this->options['currency_pos']; ?>' == 'before' ) {
									_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + '<?php echo $this->options['currency_sign']; ?>' + y );
								} else {
									_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + y + '<?php echo $this->options['currency_sign']; ?>' );
								}
							} else {
								_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + y.replace( '.00', '' ) );
							}
						}
					} else {
						$("#sln-flot-tooltip").remove();
